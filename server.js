...
// Notion 페이지 URL에서 32자리 page id를 뽑아 8-4-4-4-12 UUID 형식으로 되돌린다.
// (URL의 제목 슬러그 뒤에 대시 없이 붙는 32자 hex, 이미 대시가 있는 형태도 지원)
...
// title/rich_text 계열 속성에서 순수 텍스트를 뽑아낸다.
function plainTextFromRichTextProp(prop) {
  if (!prop) return '';
...
// "담당자_기획" 같은 속성에서 사람 이름을 뽑아낸다. 속성 타입(people/select/
// multi_select/rich_text)마다 값이 담기는 자리가 달라서 각각 다르게 처리한다.
function extractAssigneeName(prop) {
  if (!prop) return '';
  switch (prop.type) {
    case 'people':
      return (prop.people && prop.people[0] && prop.people[0].name) || '';
    case 'select':
      return (prop.select && prop.select.name) || '';
    case 'multi_select':
      return (prop.multi_select && prop.multi_select[0] && prop.multi_select[0].name) || '';
    case 'rich_text':
    case 'title':
      return plainTextFromRichTextProp(prop);
    default:
      return '';
  }
}

// GET /notion-page-info?url=... -> 해당 노션 페이지의 제목과 '담당자_기획' 값을 반환.
// "노션 URL로 등록" 버튼에서 사용: 매번 페이지를 열어 제목/링크를 옮겨 적던 걸 자동화한다.
// 이 서버의 Notion 연동(integration)에 해당 페이지가 공유돼 있어야 조회된다.
app.get('/notion-page-info', async (req, res) => {
  const pageId = extractNotionPageId(req.query.url || '');
  if (!pageId) {
    return res.status(400).json({ error: 'invalid Notion url (page id를 찾을 수 없습니다)' });
  }

  try {
    const page = await notion.pages.retrieve({ page_id: pageId });
    if (page.object !== 'page' || !page.properties) {
      return res.status(404).json({ error: 'Notion 페이지를 찾을 수 없습니다' });
    }

    const titleKey = Object.keys(page.properties).find((k) => page.properties[k].type === 'title');
    const title = titleKey ? plainTextFromRichTextProp(page.properties[titleKey]) : '';
    const assignee = extractAssigneeName(page.properties['담당자_기획']);
